...
import { getAppropriateEcmaVersionForBrowserslist, normalizeBrowserslist, browsersWithSupportForEcmaVersion } from '@wessberg/browserslist-generator';
import { createHash, createHmac } from 'crypto';
import * as TSModule from 'typescript';
import { statSync } from 'fs';
import { loadPartialConfig, createConfigItem, loadOptions, transformAsync } from '@babel/core';
import { readConfig, findConfig } from 'browserslist';
import { createFilter } from '@rollup/pluginutils';
import { camelCase, matchAll } from '@wessberg/stringutil';
import { preserveNode, cloneNode, setParentNodes } from '@wessberg/ts-clone-node';
...
function isBabelCjsHelper(p) {
    return (!includesBabelEsmHelper(p) &&
...
function pathsAreRelated(pathA, pathB) {
    if (pathA === pathB)
        return true;
    ...
    if (pathA.includes(NODE_MODULES) || pathB.includes(NODE_MODULES)) {
        ...
        if (pathAFromNodeModules.includes(pathBFromNodeModules))
            return true;
        if (pathBFromNodeModules.includes(pathAFromNodeModules))
            return true;
    }
    return false;
}
...
function getParsedCommandLine(options) {
    ...
    // Otherwise, attempt to resolve it and parse it
        ...
        tsconfigPath = ensureAbsolute(cwd, isTsConfigResolverWithFileName(tsconfig)
            ? tsconfig.fileName
        ...
        originalCompilerOptions = typescript.parseJsonConfigFileContent(typescript.parseConfigFileTextToJson(tsconfigPath, tsconfigContent).config, fileSystem, cwd, {}, tsconfigPath).options;
        parsedCommandLine = typescript.parseJsonConfigFileContent(typescript.parseConfigFileTextToJson(tsconfigPath, tsconfigContent).config, fileSystem, cwd, forcedCompilerOptions, tsconfigPath);
...
function configItemIsRelevantForChunkPhase(configItem) {
    return (BABEL_MINIFY_PRESET_NAMES.some(preset => { var _a; return (_a = configItem.file) === null || _a === void 0 ? void 0 : _a.resolved.includes(preset); }) ||
        BABEL_MINIFY_PLUGIN_NAMES.some(plugin => { var _a; return (_a = configItem.file) === null || _a === void 0 ? void 0 : _a.resolved.includes(plugin); }));
}
/**
...
function getBabelConfig({ babelConfig, cwd, forcedOptions = {}, defaultOptions = {}, browserslist, phase, hook }) {
    return (filename) => {
        ...
        const combined = Object.assign(Object.assign(Object.assign(Object.assign({}, otherDefaultOptions), options), otherForcedOptions), { presets: combineConfigItems(((_a = options.presets) !== null && _a !== void 0 ? _a : []), defaultPresets == null
                ? undefined
                : (_c = (_b = loadPartialConfig(Object.assign({ presets: defaultPresets }, configFileOption))) === null || _b === void 0 ? void 0 : _b.options.presets) !== null && _c !== void 0 ? _c : undefined, forcedPresets == null
                ? undefined
                : (_e = (_d = loadPartialConfig(Object.assign({ presets: forcedPresets }, configFileOption))) === null || _d === void 0 ? void 0 : _d.options.presets) !== null && _e !== void 0 ? _e : undefined, phase === "chunk"), plugins: combineConfigItems(((_f = options.plugins) !== null && _f !== void 0 ? _f : []), defaultPlugins == null
                ? undefined
                : (_h = (_g = loadPartialConfig(Object.assign({ plugins: defaultPlugins }, configFileOption))) === null || _g === void 0 ? void 0 : _g.options.plugins) !== null && _h !== void 0 ? _h : undefined, forcedPlugins == null
                ? undefined
                : (_k = (_j = loadPartialConfig(Object.assign({ plugins: forcedPlugins }, configFileOption))) === null || _j === void 0 ? void 0 : _j.options.plugins) !== null && _k !== void 0 ? _k : undefined, phase === "chunk") });
        ...
        const loadedOptions = (_o = loadOptions(Object.assign(Object.assign(Object.assign({}, combinedOptionsAfterHook), { filename }), configFileOption))) !== null && _o !== void 0 ? _o : undefined;
        ...
        if (phase === "chunk") {
            const hasRelevantConfigItems = loadedOptions != null &&
                [...((_p = combined.plugins) !== null && _p !== void 0 ? _p : []).filter(configItemIsRelevantForChunkPhase), ...((_q = combined.presets) !== null && _q !== void 0 ? _q : []).filter(configItemIsRelevantForChunkPhase)]
                    .length > 0;
            return {
...
function mergeTransformers(...transformers) {
    return options => {
        ...
        const afterDeclarationsTransformers = [].concat.apply([], instantiatedTransformers
...
function needsInitialize(options) {
    return (options.sourceFileToExportedSymbolSet.size === 0 ||
...
function visitExportDeclaration$1(options) {
    ...
    return [
        preserveMeta(typescript.updateExportDeclaration(node, node.decorators, node.modifiers, exportClause, node.moduleSpecifier, node.isTypeOnly), node, options),
        ...other
    ];
}
...
function generateExportDeclarations(options, exportDeclarations = []) {
    ...
    for (const symbol of exportedSymbols) {
        ...
        // Otherwise, we can just add an ExportDeclaration with an ExportSpecifier
            ...
            exportDeclarations.push(preserveParents(typescript.createExportDeclaration(undefined, undefined, typescript.createNamedExports([exportSpecifier]), symbol.moduleSpecifier == null || generatedModuleSpecifier == null || matchingSourceFile != null
                ? undefined
                : typescript.createStringLiteral(generatedModuleSpecifier)), { typescript }));
...
function visitExportDeclaration$2(options) {
    ...
    // Instead, we must declare inline the namespace and add an ExportDeclaration with a named export for it
        ...
        ]), options.typescript.NodeFlags.Namespace), options), preserveParents(options.typescript.createExportDeclaration(undefined, undefined, typescript.createNamedExports([
            typescript.createExportSpecifier(undefined, typescript.createIdentifier(contResult.exportClause.name.text))
        ]), undefined, contResult.isTypeOnly), options));
    }
...
function visitImportTypeNode$1(options) {
    var _a;
    const { node } = options;
    const moduleSpecifier = !options.typescript.isLiteralTypeNode(node.argument) || !options.typescript.isStringLiteralLike(node.argument.literal)
        ? undefined
...
function getBestDeclaration(options) {
    ...
    else if (options.typescript.isIdentifier(declaration) &&
...
function createAliasedBinding(node, propertyName, name, typescript, typeChecker, lexicalEnvironment) {
    ...
    switch (declaration === null || declaration === void 0 ? void 0 : declaration.kind) {
        ...
        case typescript.SyntaxKind.ExportAssignment: {
            return [
                preserveParents(typescript.createVariableStatement(ensureHasDeclareModifier(undefined, typescript), typescript.createVariableDeclarationList([
...
function locateExportedSymbolForSourceFile(options, context, seenSourceFiles = new Set()) {
    ...
    if ("defaultExport" in options && options.defaultExport) {
        return exportedSymbolsArr.find(exportedSymbol => "isDefaultExport" in exportedSymbol &&
...
function deconflictBindingElement(options) {
    ...
    const propertyNameContResult = node.propertyName == null
        ? undefined
...
function deconflictIdentifier(options) {
    ...
    const textResult = deconflictedBindingMapLookupResult != null && deconflictedBindingMapLookupResult.startsWith(node.text)
...
function deconflictMethodSignature(options) {
    ...
    const nextContinuationOptions = { lexicalEnvironment: cloneLexicalEnvironment(lexicalEnvironment) };
    const typeParametersContResult = node.typeParameters == null
        ? undefined
    ...
    return {
        ...
        renderChunk(code, chunk, outputOptions) {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const transpilationResult = yield transformAsync(updatedCode, Object.assign(Object.assign(Object.assign({}, config), { filenameRelative: ensureRelative(cwd, chunk.fileName) }), (updatedMap == null
                    ? {}
                    : {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (sourceDescription == null) {
                    ...
                    // Otherwise, pass it on to Babel to perform the rest of the transpilation steps
                        ...
                        const transpilationResult = yield transformAsync(sourceDescription.code, Object.assign(Object.assign({}, babelConfigResult.config), { filenameRelative: ensureRelative(cwd, file), inputSourceMap: typeof sourceDescription.map === "string" ? JSON.parse(sourceDescription.map) : sourceDescription.map }));
                        if (transpilationResult == null || transpilationResult.code == null) {
                            return sourceDescription;
                        }
                        // Return the results
        ...
        generateBundle(outputOptions, bundle) {
            ...
            if (Boolean(parsedCommandLineResult.parsedCommandLine.options.incremental) ||
...
export default typescriptRollupPlugin;
